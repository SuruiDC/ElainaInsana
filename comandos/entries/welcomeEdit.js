module.exports.run = async (bot, message, args) => {
    ...
    switch(param.toLowerCase()){
        ...
        await data.updateOne({ "welcomeConfig.author": setting })
        ...
        if(image) {
            ...
            if(["{member.image}","{guild.image}"].some(x => x.toLowerCase() === setting)){
                await data.updateOne({ "welcomeConfig.authorImage": setting.toLowerCase() })
                return message.channel.send("Se actualizo el `authorImage`.")
            } 
        ...
        await data.updateOne({ "welcomeConfig.description": setting })
        ...
        await data.updateOne({ "welcomeConfig.image": image.url })
        ...
        if(image) {
            ...
            if(!setting) return message.channel.send("Debes incrustar una image al mensaje o puedes escribir las variables `{guild.image}` o `{member.image}`.")
            if(["{member.image}","{guild.image}"].some(x => x.toLowerCase() === setting)){
                await data.updateOne({ "welcomeConfig.thumbnail": setting.toLowerCase() })
                return message.channel.send("Se actualizo el `thumbnail`.")
            } 
        ...
        await data.updateOne({ "welcomeConfig.footer": setting })
        ...
        if(image) {
            ...
            if(!setting) return message.channel.send("Debes incrustar una image al mensaje o puedes escribir las variables `{guild.image}` o `{member.image}`.")
            if(["{member.image}","{guild.image}"].some(x => x.toLowerCase() === setting)){
                await data.updateOne({ "welcomeConfig.footerImage": setting.toLowerCase() })
                return message.channel.send("Se actualizo el `footerImage`.")
            } 
        ...
        await data.updateOne({ "welcomeConfig.color": setting })
        ...
        if(setting.toLowerCase() === "on") {
            await data.updateOne({ "welcomeConfig.timestamp": true})
        } else {
            await data.updateOne({ "welcomeConfig.timestamp": false})
        }
        ...
        await data.updateOne({ "welcomeConfig.title": setting })
...
exports.conf = {
    ...
    exports.help = {
    name: "welcomeedit",
    description: "Configurar el mensaje embed de las bienvenidas.", 
    usage: "e!welcomeedit [`parametro`]",
    cooldown: 2
}